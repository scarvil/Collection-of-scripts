#!/bin/bash
# Name: DeleteCheckPointBackups.sh 
# By: scarvill@company.com
# Date : 09/22/2010
# Revision: Initial
#
# Vars
...
# Check the timestamp on the 2nd latest checkpoint file
...
if [ -f $FIND_RECENT_CKP ]; then 
	...
	if [ $TIME_OF_LATEST_FILE_DAY = $CURRENT_DAY ]; then
		echo "The last checkpoint for $FIND_RECENT_CKP is current." >> $LOG
                # Is the 2nd latest checkpoint there?
                # If so, delete all checkpoints over 2 days old.
		...
		# If the checkpoint is over a day old and the most recent, keep it.
		echo "" >> $LOG
