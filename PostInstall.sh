#!/bin/bash
IP=192.168.1.2
# Install pp-jce-jdk1.8.0_73-1.0-0.el7.x86_64.rpm
wget http://$IP/kickstart/extras/pp-jce-jdk1.8.0_73-1.0-0.el7.x86_64.rpm
rpm -ivh  pp-jce-jdk1.8.0_73-1.0-0.el7.x86_64.rpm 
rm -rf pp-jce-jdk1.8.0_73-1.0-0.el7.x86_64.rpm
...
[pp-hsre-misc]
name=pp-hsre-misc
baseurl=http://bogushost.blahblah.com/repo/pp-hsre/misc/\$releasever/\$basearch
gpgcheck=0
enabled=1
priority=1
...
[pp-hsre-jdk]
name=pp-hsre-jdk
baseurl=http://bogushost.blahblah.com/repo/pp-hsre/jdk/\$releasever/\$basearch
gpgcheck=0
enabled=1
priority=1
...
[pp-hsre-ruby]
name=pp-hsre-ruby
baseurl=http://bogushost.blahblah.com/repo/pp-hsre/ruby/\$releasever/\$basearch
gpgcheck=0
enabled=1
priority=1
...
# Add /etc/sysctl.d/101-bdp.conf
cat<<EOF > /etc/sysctl.d/101-bdp.conf
...
#echo "PEERDNS=no" >> ifcfg-bond0_slave_1
#echo "PEERDNS=no" >> ifcfg-bond0_slave_2
#echo "PEERDNS=no" >> ifcfg-bond0
# Remove NETBOOT entry in primary kickstart device
#sed -i -e '/NETBOOT=yes/d' ifcfg-p1p1
...
echo " " >> /tmp/\$FILE
echo "=== pp-jce-jdk1.8.0_73-1.0-0.el7.x86_64 ===" >> /tmp/\$FILE
rpm -q pp-jce-jdk1.8.0_73-1.0-0.el7.x86_64 >> /tmp/\$FILE
echo " " >> /tmp/\$FILE
echo "=== df ===" >> /tmp/\$FILE
...
echo " " >> /tmp/\$FILE
echo "=== cat /etc/yum.repos.d/hsre.repo ===" >> /tmp/\$FILE
cat /etc/yum.repos.d/hsre.repo >> /tmp/\$FILE
echo " " >> /tmp/\$FILE
echo "=== cat /etc/yum.repos.d/zabbix.repo ===" >> /tmp/\$FILE
cat /etc/yum.repos.d/zabbix.repo >> /tmp/\$FILE
echo " " >> /tmp/\$FILE
echo "=== cat /etc/sysctl.d/101-bdp.conf ===" >> /tmp/\$FILE
cat /etc/sysctl.d/101-bdp.conf >> /tmp/\$FILE
echo " " >> /tmp/\$FILE
